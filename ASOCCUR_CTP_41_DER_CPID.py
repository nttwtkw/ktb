...
def CTP41CPID_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_CPID')
    ...
    #query from db2
    query_db2 = config['RDT040101']

    #query check prevalidate
    query_pre_validation = function.replace_value(config['RDT040102'], ['{CODE_DB2}'], [query_db2])
...
def CTP41CPID_AP01_API_DER_CPID():
    try:
        #read config files
        config = function.read_config_psql('DER_CPID')
    except Exception as e:
...
def CTP41CPID_VALD01():
    ...
    try:
        ...
        result_CNCPID001 = pd.read_sql(config['RDT040103'], conn_psql)

        #Tax Id must be a valid number according to check digit algorithm.
        result_CNCPID002 = pd.read_sql(config['RDT040104'], conn_psql) 

        #Juristic Id must be a valid number according to check digit algorithm.
        result_CNCPID003 = pd.read_sql(config['RDT040105'], conn_psql) 

        #Cooperative Id must be a valid number according to check digit algorithm.
        result_CNCPID004 = pd.read_sql(config['RDT040106'], conn_psql) 

        #If [Identification Type] is “Offshore Branch Number”, then [Branch FI Code] cannot be blank and [Branch FI Code] must be FI Code.
        result_CMCPID001 = pd.read_sql(config['RDT040107'], conn_psql) 

        #If an identification type of juristic person or ordinary person is not exempt, then a juristic counterparty or an ordinary counterparty must be reported.
        result_CMCPID002 = pd.read_sql(config['RDT040108'], conn_psql)
        
        #If [Application Date] in Application (DER_APP) is later than or equal to ‘2023-10-01’, then [Customer Identification Type Country] must not be "unclassified".
        result_CMCPID003 = pd.read_sql(config['RDT040109'], conn_psql) 
    ...
    #validation success
    if (result_CNCPID001['Counterparty Id'].count() == 0 and
        result_CNCPID002['Counterparty Id'].count() == 0 and
        result_CNCPID003['Counterparty Id'].count() == 0 and
        result_CNCPID004['Counterparty Id'].count() == 0 and
        result_CMCPID001['Counterparty Id'].count() == 0 and
        result_CMCPID002['Counterparty Id'].count() == 0 and
        result_CMCPID003['Counterparty Id'].count() == 0):
        ...
        try:
            if result_CNCPID001['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CNCPID001,'{0}{1}/CTP41CPID_CNCPID001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP41CPID_CNCPID001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CNCPID002['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CNCPID002,'{0}{1}/CTP41CPID_CNCPID002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP41CPID_CNCPID002.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CNCPID003['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CNCPID003,'{0}{1}/CTP41CPID_CNCPID003.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP41CPID_CNCPID003.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CNCPID004['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CNCPID004,'{0}{1}/CTP41CPID_CNCPID004.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP41CPID_CNCPID004.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMCPID001['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCPID001,'{0}{1}/CTP41CPID_CMCPID001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP41CPID_CMCPID001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMCPID002['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCPID002,'{0}{1}/CTP41CPID_CMCPID002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP41CPID_CMCPID002.csv fail Line", exc_tb.tb_lineno)
        try:
            if result_CMCPID003['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCPID003,'{0}{1}/CTP41CPID_CMCPID003.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP41CPID_CMCPID003.csv fail Line", exc_tb.tb_lineno)
...
def CTP41CPID_GEN01():
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT040110'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('ASOCCUR_CTP_41_DER_CPID',
         schedule_interval='@daily',
         default_args=default_args,
         description='As Occur: 4.1 Counterparty x Id',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CTP41CPID_LOAD01',
        python_callable=CTP41CPID_LOAD01
    )

    task2 = PythonOperator(
        task_id='CTP41CPID_XFRM01',
        python_callable=CTP41CPID_XFRM01
    )

    task3 = PythonOperator(
        task_id='CTP41CPID_AP01_API_DER_CPID',
        python_callable=CTP41CPID_AP01_API_DER_CPID
    )

    task4 = PythonOperator(
        task_id='CTP41CPID_VALD01',
        python_callable=CTP41CPID_VALD01
    ) 

    task5 = PythonOperator(
        task_id='CTP41CPID_GEN01',
        python_callable=CTP41CPID_GEN01
    )
