...
def CTP42JCP_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_JCP')
    ...
    #query from db2
    query_db2 = config['RDT040201']

    #query check prevalidate
    query_pre_validation = function.replace_value(config['RDT040202'], ['{CODE_DB2}'], [query_db2])
...
def CTP42JCP_VALD01():    
    ...
    try:
        ...
        #If [Registered Country] is ‘TH’, then year of [Juristic Date] must be greater than 1753.
        result_CMJCP002 = pd.read_sql(config['RDT040203'] ,conn_psql) 

        #Either [Registered Business Name Thai] or [Registered Business Name English] cannot be blank.
        result_CMJCP003 = pd.read_sql(config['RDT040204'] ,conn_psql)
        
        #If [Data Date] is later than or equal to ‘2023-10-01’, then [Registered Country] and [Nationality of Headquarter] must not be "unclassified".
        result_CMJCP004 = pd.read_sql(config['RDT040205'] ,conn_psql)

        #If a juristic counterparty record exists, then a counterparty x id record for the juristic counterparty must exist.
        result_RIJCP001 = pd.read_sql(config['RDT040206'] ,conn_psql) 
    ...
    #validation success
    if (result_CMJCP002['Counterparty Id'].count() == 0 and
        result_CMJCP003['Counterparty Id'].count() == 0 and
        result_CMJCP004['Counterparty Id'].count() == 0 and
        result_RIJCP001['Counterparty Id'].count() == 0 ):
        ...
        try:
            if result_CMJCP002['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CMJCP002,'{0}{1}/CTP42JCP_CMJCP002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP42JCP_CMJCP002.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMJCP003['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CMJCP003,'{0}{1}/CTP42JCP_CMJCP003.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP42JCP_CMJCP003.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMJCP004['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CMJCP004,'{0}{1}/CTP42JCP_CMJCP004.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP42JCP_CMJCP004.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RIJCP001['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_RIJCP001,'{0}{1}/CTP42JCP_RIJCP001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP42JCP_RIJCP001.csv fail Line", exc_tb.tb_lineno)
...
def CTP42JCP_GEN01():
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT040207'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('ASOCCUR_CTP_42_DER_JCP',
         schedule_interval='@daily',
         default_args=default_args,
         description='As Occur: 4.2 Juristic Counterparty',
         catchup=False) as dag:
 
    task1 = PythonOperator(
    task_id='CTP42JCP_LOAD01',
    python_callable=CTP42JCP_LOAD01
    )

    task2 = PythonOperator(
    task_id='CTP42JCP_XFRM01',
    python_callable=CTP42JCP_XFRM01  
    )

    task3 = PythonOperator(
    task_id='CTP42JCP_AP01_API_DER_JCP',
    python_callable=CTP42JCP_AP01_API_DER_JCP  
    )
        
    task4 = PythonOperator(
    task_id='CTP42JCP_VALD01',
    python_callable=CTP42JCP_VALD01
    ) 

    task5 = PythonOperator(
    task_id='CTP42JCP_GEN01',
    python_callable=CTP42JCP_GEN01 
    )
