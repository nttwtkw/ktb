...
def CMM74OTDD_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_OTDD_D')
...
def CMM74OTDD_VALD01():
    ...
    try:
        ...
        #If a credit account is Thai Baht loans, credit card loans, contingent loans or loans from an offshore branch of the Thai commercial bank, then an outstanding daily record of this credit account is not required to report.
        result_CNOTDD001 = pd.read_sql(config['RDT070404'], conn_psql)

        #[Outstanding Amount in Original Currency] must be greater than or equal to 0
        result_CNOTDD002 = pd.read_sql(config['RDT070405'], conn_psql) 

        #If a credit account is closed, then an outstanding daily record of this credit account is not required to report.
        result_CNOTDD003 = pd.read_sql(config['RDT070406'], conn_psql) 

        #Sum up of [Outstanding Amount in Original Currency] in Outstanding Daily (DER_OTDD) must be greater than or equal to Sum up of [Foreign Currency Amount] in Foreign Currency Position (DS_FCP) with conditions of on-shore lending.
        result_CMOTDD001 = pd.read_sql(config['RDT070407'], conn_psql) 

        #Sum up of [Outstanding Amount in Original Currency] in Outstanding Daily (DER_OTDD) must be greater than or equal to Sum up of [Foreign Currency Amount] in Foreign Currency Position (DS_FCP) with conditions of off-shore lending.
        result_CMOTDD002 = pd.read_sql(config['RDT070408'], conn_psql) 

        #Sum up of [Outstanding Amount in Original Currency] must be equal to Sum up of [Foreign Currency Amount] in Foreign Currency Position (DS_FCP) with conditions of overdraft from deposit account.
        result_CMOTDD003 = pd.read_sql(config['RDT070409'], conn_psql) 

        #Sum up of [Outstanding Amount in Original Currency] in Outstanding Daily (DER_OTDD) must be greater than or equal to Sum up of [Foreign Currency Amount] in Foreign Currency Position (DS_FCP) with conditions of foreign bill inward negotiated.
        result_CMOTDD004 = pd.read_sql(config['RDT070410'], conn_psql) 
        
        #Sum up of [Outstanding Amount in Original Currency] in Outstanding Daily (DER_OTDD) must be equal to Sum up of [Foreign Currency Amount] in Foreign Currency Position (DS_FCP) with conditions of foreign bill outward negotiated.
        result_CMOTDD005 = pd.read_sql(config['RDT070411'], conn_psql)
        
        #If an outstanding daily record exists, then a credit account record must exist.
        result_RIOTDD001 = pd.read_sql(config['RDT070412'], conn_psql)
    ...
    if (result_CNOTDD001['Account Id'].count() == 0 
        and result_CNOTDD002['Account Id'].count() == 0
        and result_CNOTDD003['Account Id'].count() == 0
        and result_CMOTDD001['Account Id'].count() == 0
        and result_CMOTDD002['Account Id'].count() == 0
        and result_CMOTDD003['Account Id'].count() == 0
        and result_CMOTDD004['Account Id'].count() == 0
        and result_CMOTDD005['Account Id'].count() == 0
        and result_RIOTDD001['Account Id'].count() == 0):
        ...
        try:
            if result_CNOTDD001['Account Id'].count() != 0:
                entity_rule = 'CMM74OTDD_CNOTDD001.csv'
                function.upload_validation_fail_by_source_to_minio(result_CNOTDD001, config['VALD_PATH'], entity_rule, unixtime, chunksize, s3, bucket_name, table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CNOTDD002['Account Id'].count() != 0:
                entity_rule = 'CMM74OTDD_CNOTDD002.csv'
                function.upload_validation_fail_by_source_to_minio(result_CNOTDD002, config['VALD_PATH'], entity_rule, unixtime, chunksize, s3, bucket_name, table)            
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CNOTDD003['Account Id'].count() != 0:
                entity_rule = 'CMM74OTDD_CNOTDD003.csv'
                function.upload_validation_fail_by_source_to_minio(result_CNOTDD003, config['VALD_PATH'], entity_rule, unixtime, chunksize, s3, bucket_name, table)            
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMOTDD001['Account Id'].count() != 0:
                entity_rule = 'CMM74OTDD_CMOTDD001.csv'
                function.upload_validation_fail_by_source_to_minio(result_CMOTDD001, config['VALD_PATH'], entity_rule, unixtime, chunksize, s3, bucket_name, table)            
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMOTDD002['Account Id'].count() != 0:
                entity_rule = 'CMM74OTDD_CMOTDD002.csv'
                function.upload_validation_fail_by_source_to_minio(result_CMOTDD002, config['VALD_PATH'], entity_rule, unixtime, chunksize, s3, bucket_name, table)            
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMOTDD003['Account Id'].count() != 0:
                entity_rule = 'CMM74OTDD_CMOTDD003.csv'
                function.upload_validation_fail_by_source_to_minio(result_CMOTDD003, config['VALD_PATH'], entity_rule, unixtime, chunksize, s3, bucket_name, table)            
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMOTDD004['Account Id'].count() != 0:
                entity_rule = 'CMM74OTDD_CMOTDD004.csv'
                function.upload_validation_fail_by_source_to_minio(result_CMOTDD004, config['VALD_PATH'], entity_rule, unixtime, chunksize, s3, bucket_name, table)            
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMOTDD005['Account Id'].count() != 0:
                entity_rule = 'CMM74OTDD_CMOTDD005.csv'
                function.upload_validation_fail_by_source_to_minio(result_CMOTDD005, config['VALD_PATH'], entity_rule, unixtime, chunksize, s3, bucket_name, table)            
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_RIOTDD001['Account Id'].count() != 0:
                entity_rule = 'CMM74OTDD_RIOTDD001.csv'
                function.upload_validation_fail_by_source_to_minio(result_RIOTDD001, config['VALD_PATH'], entity_rule, unixtime, chunksize, s3, bucket_name, table)            
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
...
def CMM74OTDD_GEN01():
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT070413'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('DAILY_CMM_74_DER_OTDD',
