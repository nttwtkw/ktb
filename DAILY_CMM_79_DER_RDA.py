...
def CMM79RDA_LOAD01():
    ...
    try:
        ...
        df,con_db2 = function.read_psql(config['RDT070902'])
...
def CMM79RDA_VALD01():
    ...
    try:
        ...
        #If a related deposit account record exists, then a transaction flow record must exist, as the two data entities describe the element of transaction.
        result_RIRDA001 = pd.read_sql(config['RDT070903'], conn_psql) 
    ...
    #validation success
    if (result_RIRDA001['Transaction Id'].count() == 0 ):
        ...
        try:
            if result_RIRDA001['Transaction Id'].count() != 0:
                entity_rule = 'CMM79RDA_RIRDA001.csv'
                function.upload_validation_fail_by_source_to_minio(result_RIRDA001, config['VALD_PATH'], entity_rule, unixtime, chunksize, s3, bucket_name, table)            
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CMM79RDA_RIRDA001.csv fail Line", exc_tb.tb_lineno)
...
def CMM79RDA_GEN01():
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT070904'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('DAILY_CMM_79_DER_RDA',
         schedule_interval='@daily', 
         default_args=default_args,
         description='Daily: 7.9 Related Deposit Account',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CMM79RDA_LOAD01',
        python_callable=CMM79RDA_LOAD01
    )

    task2 = PythonOperator(
        task_id='CMM79RDA_XFRM01',
        python_callable=CMM79RDA_XFRM01    
    )

    task3 = PythonOperator(
        task_id='CMM79RDA_AP01_DPI_DER_RDA',
        python_callable=CMM79RDA_AP01_DPI_DER_RDA      
    ) 

    task4 = PythonOperator(
        task_id='CMM79RDA_VALD01',
        python_callable=CMM79RDA_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='CMM79RDA_GEN01',
        python_callable=CMM79RDA_GEN01
    )
