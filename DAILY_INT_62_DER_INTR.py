...
def INT62INTR_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_INTR_D')
...
def INT62INTR_AP01_DPI_DER_INTR():
    try:
        #read config files
        config = function.read_config_psql('DER_INTR_D')
    except Exception as e:
...
def INT62INTR_VALD01():
    ...
    try:
        ...
        result_CMINTR001 = pd.read_sql(config['RDT060204'], conn_psql)
        ...
        result_CMINTR002 = pd.read_sql(config['RDT060205'], conn_psql) 
    ...
    if (result_CMINTR001['Reference Rate Id'].count() == 0 
        and result_CMINTR002['Reference Rate Id'].count() == 0):
        ...
        try:
            if result_CMINTR001['Reference Rate Id'].count() != 0:
                entity_rule = 'INT62INTR_CMINTR001.csv'
                function.upload_validation_fail_by_source_to_minio(result_CMINTR001, config['VALD_PATH'], entity_rule, unixtime, chunksize, s3, bucket_name, table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio INT62INTR_CMINTR001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMINTR002['Reference Rate Id'].count() != 0:
                entity_rule = 'INT62INTR_CMINTR002.csv'
                function.upload_validation_fail_by_source_to_minio(result_CMINTR002, config['VALD_PATH'], entity_rule, unixtime, chunksize, s3, bucket_name, table)            
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio INT62INTR_CMINTR002.csv fail Line", exc_tb.tb_lineno)
...
def INT62INTR_GEN01():
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT060206'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('DAILY_INT_62_DER_INTR',
         schedule_interval='@daily', 
         default_args=default_args,
         description='Daily: 6.2 Interest Reference',
    ...
    task3 = PythonOperator(
        task_id='INT62INTR_AP01_DPI_DER_INTR',
        python_callable=INT62INTR_AP01_DPI_DER_INTR      
    ) 
