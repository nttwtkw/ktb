...
def INT62INTR_LOAD_TFS01():
    ...
    #update batch     
    task_name = config['TASK_NAME_TFS_01']
    number_id_job = '1'
    date_process = config['PROCESS_START_DATE']
    status = config['STATUS_RUN']
    function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID_TFS'],number_id_job,date_process,status,True)
    ...
    #parameter
    path_sftp = config['TFS_PATH']
    name_files_sftp = config['SFTP_NAME_TFS']
    msr_prd_id = config['MSR_PRD_ID_TFS']
    table = config['TABLE_TFS']
    ...
    function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID_TFS'],number_id_job,date_process,status,False)
...
with DAG('DAILY_INT_62_DER_INTR_TFS',
         schedule_interval='@daily', 
         default_args=default_args,
         description='Daily: 6.2 Interest Reference TFS',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='INT62INTR_LOAD_TFS01',
        python_callable=INT62INTR_LOAD_TFS01
    )
