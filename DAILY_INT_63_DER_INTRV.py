...
def INT63INTRV_LOAD01():
    ...
    try:
        ...
        df,con_db2 = function.read_psql(config['RDT060302'])
...
def INT63INTRV_AP01_DPI_DER_INTRV():
    try:
        #read config files
        config = function.read_config_psql('DER_INTRV_D')
    except Exception as e:
...
def INT63INTRV_VALD01():
    ...
    try:
        ...
        #If an interest reference value record exists, then an interest reference record must exist.
        result_RIINTRV001 = pd.read_sql(config['RDT060303'], conn_psql)
    ...
    #validation success
    if (result_RIINTRV001['Reference Rate Id'].count() == 0 ):
        ...
        try:
            if result_RIINTRV001['Reference Rate Id'].count() != 0:
                entity_rule = 'INT63INTRV_RIINTRV001.csv'
            ...
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio INT63INTRV_RIINTRV001.csv fail Line", exc_tb.tb_lineno)

    #update batch   
    status = config['STATUS_SUCCESS']
    function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,False)
    
def INT63INTRV_GEN01():
    try:
        #read config files
        config = function.read_config_psql('DER_INTRV_D')
    except Exception as e:
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT060304'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('DAILY_INT_63_DER_INTRV',
    ...
    task1 = PythonOperator(
        task_id='INT63INTRV_LOAD01',
        python_callable=INT63INTRV_LOAD01
    )

    task2 = PythonOperator(
        task_id='INT63INTRV_XFRM01',
        python_callable=INT63INTRV_XFRM01    
    )

    task3 = PythonOperator(
        task_id='INT63INTRV_AP01_DPI_DER_INTRV',
        python_callable=INT63INTRV_AP01_DPI_DER_INTRV      
    ) 

    task4 = PythonOperator(
        task_id='INT63INTRV_VALD01',
        python_callable=INT63INTRV_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='INT63INTRV_GEN01',
        python_callable=INT63INTRV_GEN01
    )
