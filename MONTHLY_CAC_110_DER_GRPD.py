...
def CAC110GRPD_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_GRPD')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT011001'])
...
def CAC110GRPD_AP01_MPI_DER_GRPD():
    try:
        #read config files
        config = function.read_config_psql('DER_GRPD')
    except Exception as e:
...
def CAC110GRPD_VALD01():
    ...
    try:
        ...
        #[Effective Date] must be after of the same as [Effective Date] in Credit Account (DER_CAC).
        result_CNGRPD001 = pd.read_sql(config['RDT011002'] ,conn_psql) 

        #[End Date] must be after or the same as [Effective Date].
        result_CNGRPD002 = pd.read_sql(config['RDT011003'] ,conn_psql)

        #If [Grace Period Type] is “Other grace period”, then [Other Grace Period Type Description] must describe.
        result_CMGRPD001 = pd.read_sql(config['RDT011004'] ,conn_psql) 

        #If a grace period record exists, then a credit account record must exist.
        result_RIGRPD001 = pd.read_sql(config['RDT011005'] ,conn_psql) 
    ...
    #validation success
    if (result_CNGRPD001['Account Id'].count() == 0 and
        result_CNGRPD002['Account Id'].count() == 0 and
        result_CMGRPD001['Account Id'].count() == 0 and
        result_RIGRPD001['Account Id'].count() == 0):
        ...
        try:
            if result_CNGRPD001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CNGRPD001,'{0}{1}/CAC110GRPD_CNGRPD001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC110GRPD_CNGRPD001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CNGRPD002['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CNGRPD002,'{0}{1}/CAC110GRPD_CNGRPD002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC110GRPD_CNGRPD002.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMGRPD001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CMGRPD001,'{0}{1}/CAC110GRPD_CMGRPD001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC110GRPD_CMGRPD001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RIGRPD001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_RIGRPD001,'{0}{1}/CAC110GRPD_RIGRPD001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC110GRPD_RIGRPD001.csv fail Line", exc_tb.tb_lineno)

    #update batch   
    status = config['STATUS_SUCCESS']
    function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status)          

def CAC110GRPD_GEN01(): 
    try:
        #read config files
        config = function.read_config_psql('DER_GRPD')
    except Exception as e:
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT011006'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_CAC_110_DER_GRPD',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 1.10 Grace Period',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CAC110GRPD_LOAD01',
        python_callable=CAC110GRPD_LOAD01
    )
    
    task2 = PythonOperator(
        task_id='CAC110GRPD_XFRM01',
        python_callable=CAC110GRPD_XFRM01
    )

    task3 = PythonOperator(
        task_id='CAC110GRPD_AP01_MPI_DER_GRPD',
        python_callable=CAC110GRPD_AP01_MPI_DER_GRPD    
    )

    task4 = PythonOperator(
        task_id='CAC110GRPD_VALD01',
        python_callable=CAC110GRPD_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='CAC110GRPD_GEN01',
        python_callable=CAC110GRPD_GEN01      
    ) 
