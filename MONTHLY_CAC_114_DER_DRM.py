...
def CAC114DRM_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_DRM')
    ...
    #query from db2
    query_db2 = config['RDT011401']

    #query check prevalidate
    query_pre_validation = function.replace_value(config['RDT011402'], ['{CODE_DB2}'], [query_db2])
...
def CAC114DRM_AP01_MPI_DER_DRM():
    try:
        #read config files
        config = function.read_config_psql('DER_DRM')
    except Exception as e:
...
def CAC114DRM_VALD01():
    ...
    try:
        ...
        #If DR Method is “other method”, then [Other DR Method Description] must describe.
        result_CNDRM001 = pd.read_sql(config['RDT011403'] ,conn_psql)

        #If a debt restructuring method record exists, then a debt restructuring record must exist as the two data entities describe the element of debt restructuring.
        result_RIDRM001 = pd.read_sql(config['RDT011404'] ,conn_psql) 
    ...
    #validation success
    if ( result_CNDRM001['Account Id'].count() == 0 and
         result_RIDRM001['Account Id'].count() == 0 ):
        ...
        try:
            if result_CNDRM001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CNDRM001,'{0}{1}/CAC114DRM_CNDRM001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name, table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC114DRM_CNDRM001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RIDRM001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_RIDRM001,'{0}{1}/CAC114DRM_RIDRM001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name, table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC114DRM_RIDRM001.csv fail Line", exc_tb.tb_lineno)
...
def CAC114DRM_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT011405'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_CAC_114_DER_DRM',
         schedule_interval='@monthly',
         default_args=default_args,
         description='Monthly : 1.14 Debt Restructuring Method',
         catchup=False) as dag:
 
    task_1 = PythonOperator(
        task_id='CAC114DRM_LOAD01',
        python_callable=CAC114DRM_LOAD01
    )

    task_2 = PythonOperator(
        task_id='CAC114DRM_XFRM01',
        python_callable=CAC114DRM_XFRM01
        
    )

    task_3 = PythonOperator(
        task_id='CAC114DRM_AP01_MPI_DER_DRM',
        python_callable=CAC114DRM_AP01_MPI_DER_DRM
        
    )

    task_4 = PythonOperator(
        task_id='CAC114DRM_VALD01',
        python_callable=CAC114DRM_VALD01
        
    )

    task_5 = PythonOperator(
        task_id='CAC114DRM_GEN01',
        python_callable=CAC114DRM_GEN01
