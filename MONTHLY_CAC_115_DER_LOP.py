...
def CAC115LOP_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_LOP')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT011501'])
...
def CAC115LOP_AP01_MPI_DER_LOP():
    try:
        #read config files
        config = function.read_config_psql('DER_LOP')
    except Exception as e:
...
def CAC115LOP_VALD01():
    ...
    try:
        ...
        #[Legal Operation Date] must be after or the same as [Effective Date] in Credit Account (DER_CAC).
        result_CNLOP001 = pd.read_sql(config['RDT011502'] ,conn_psql) 

        #If a legal operation record exists, then a credit account record must exist.
        result_RILOP001 = pd.read_sql(config['RDT011503'] ,conn_psql) 
    ...
    #validation success
    if (result_CNLOP001['Account Id'].count() == 0 and
        result_RILOP001['Account Id'].count() == 0 ):
        ...
        try:
            if result_CNLOP001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CNLOP001,'{0}{1}/CAC115LOP_CNLOP001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC115LOP_CNLOP001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RILOP001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_RILOP001,'{0}{1}/CAC115LOP_RILOP001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC115LOP_RILOP001.csv fail Line", exc_tb.tb_lineno)

    #update batch   
    status = config['STATUS_SUCCESS']
    function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status)
        
def CAC115LOPL_GEN01():
    try:
        #read config files
        config = function.read_config_psql('DER_LOP')
    except Exception as e:
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT011504'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_CAC_115_DER_LOP',
         schedule_interval='@monthly',
         default_args=default_args,
         description='Monthly: 1.5 Legal Operation',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CAC115LOP_LOAD01',
        python_callable=CAC115LOP_LOAD01
    )

    task2 = PythonOperator(
        task_id='CAC115LOP_XFRM01',
        python_callable=CAC115LOP_XFRM01
    )

    task3 = PythonOperator(
        task_id='CAC115LOP_AP01_MPI_DER_LOP',
        python_callable=CAC115LOP_AP01_MPI_DER_LOP
    )

    task4 = PythonOperator(
        task_id='CAC115LOP_VALD01',
        python_callable=CAC115LOP_VALD01
    ) 

    task5 = PythonOperator(
        task_id='CAC115LOPL_GEN01',
        python_callable=CAC115LOPL_GEN01
    )
