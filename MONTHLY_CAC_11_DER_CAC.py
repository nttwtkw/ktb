...
def CAC11CAC_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_CAC')
    ...
    #query from db2
    query_db2 = config['RDT010101']

    #query check prevalidate
    query_pre_validation = function.replace_value(config['RDT010102'], ['{CODE_DB2}'], [query_db2])
...
def CAC11CAC_VALD01(): 
    ...
    try:
        ...
        #If [Share Lending Total Contract Amount in Original Currency] is not blank, then it must be greater than 0.
        result_CNCAC001 = pd.read_sql(config['RDT010103'] ,conn_psql) 

        #[Contract Amount in Original Currency] must be greater than or equal to 0.
        result_CNCAC002 = pd.read_sql(config['RDT010104'] ,conn_psql) 
        ...
        result_CMCAC001 = pd.read_sql(config['RDT010105'] ,conn_psql) 

        #If [Loan and Contingent Type] is not classified as “Contingent loans”, then [Account Term Type] cannot be blank.
        result_CMCAC002 = pd.read_sql(config['RDT010106'] ,conn_psql) 

        #If [Account Term Type] is classified as “Fixed Term”, [Loan and Contingent Type] is not classified as “Contingent loans” and [Principal Payment Frequency Unit] is not “Unknown Term Unit”, then [Number of Principal Payment] cannot be blank and must be greater than 0 as well as [Principal Payment Frequency] cannot be blank and must be greater than or equal to 0.
        result_CMCAC003 = pd.read_sql(config['RDT010107'] ,conn_psql) 

        #If [Account Term Type] is classified as “Fixed Term” and [Loan and Contingent Type] is not classified as “Contingent loans”, then [Principal Payment Frequency Unit] and [Interest Payment Frequency Unit] cannot be blank.
        result_CMCAC004 = pd.read_sql(config['RDT010108'] ,conn_psql) 

        #If [Account Term Type] is classified as “Fixed Term”, [Loan and Contingent Type] is not classified as “Contingent loans” and [Interest Payment Frequency Unit] is not “Unknown Term Unit”, then [Number of Interest Payment] cannot be blank and must be greater than 0 and [Interest Payment Frequency] cannot be blank and must be greater than or equal to 0.
        result_CMCAC005 = pd.read_sql(config['RDT010109'] ,conn_psql) 
        ...
        result_CMCAC006 = pd.read_sql(config['RDT010110'] ,conn_psql) 
        
        #If [Loan and Contingent Type] is classified as “Bills of exchange”, then [Been Extended Flag] cannot be blank.
        result_CMCAC007 = pd.read_sql(config['RDT010111'] ,conn_psql) 
    ...
    if (result_CNCAC001['Account Id'].count() == 0 and
        ...
        try:
            if result_CNCAC001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CNCAC001,'{0}{1}/CAC11CAC_CNCAC001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CNCAC002['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CNCAC002,'{0}{1}/CAC11CAC_CNCAC002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMCAC001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCAC001,'{0}{1}/CAC11CAC_CMCAC001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMCAC002['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCAC002,'{0}{1}/CAC11CAC_CMCAC002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMCAC003['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCAC003,'{0}{1}/CAC11CAC_CMCAC003.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMCAC004['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCAC004,'{0}{1}/CAC11CAC_CMCAC004.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMCAC005['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCAC005,'{0}{1}/CAC11CAC_CMCAC005.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMCAC006['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCAC006,'{0}{1}/CAC11CAC_CMCAC006.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMCAC007['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCAC007,'{0}{1}/CAC11CAC_CMCAC007.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMCAC008['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCAC008,'{0}{1}/CAC11CAC_CMCAC008.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_CMCAC009['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCAC009,'{0}{1}/CAC11CAC_CMCAC009.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        ...
        try:
            if result_RICAC001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_RICAC001,'{0}{1}/CAC11CAC_RICAC001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
...
def CAC11CAC_GEN01():
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT010115'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_CAC_11_DER_CAC',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 1.1 Credit Account',
    ...
    task3 = PythonOperator(
        task_id='CAC11CAC_AP01_MPI_DER_CAC',
        python_callable=CAC11CAC_AP01_MPI_DER_CAC      
    ) 
