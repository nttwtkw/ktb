...
def CAC15PJF_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_PJF')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT010501'])
...
def CAC15PJF_VALD01():  
    ...
    try:
        ...
        #[Estimated Project Value in Baht] must be greater than 0.
        result_CNPJF001 = pd.read_sql(config['RDT010503'] ,conn_psql) 

        #If [Project Country] is ‘TH’, then [Project Location] cannot be blank.
        result_CMPJF001 = pd.read_sql(config['RDT010504'] ,conn_psql) 

        #If a project finance record exists, then a credit account record for the project finance must exist.
        result_RIPJF001 = pd.read_sql(config['RDT010505'] ,conn_psql) 
    ...
    #validation success
    if (result_CNPJF001['Account Id'].count() == 0 and
        result_CMPJF001['Account Id'].count() == 0 and
        result_RIPJF001['Account Id'].count() == 0 ):
        ...
        try:
            if result_CNPJF001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CNPJF001,'{0}{1}/CAC15PJF_CNPJF001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC15PJF_CNPJF001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMPJF001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CMPJF001,'{0}{1}/CAC15PJF_CMPJF001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC15PJF_CMPJF001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RIPJF001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_RIPJF001,'{0}{1}/CAC15PJF_RIPJF001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC15PJF_RIPJF001.csv fail Line", exc_tb.tb_lineno)
...
def CAC15PJF_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT010506'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_CAC_15_DER_PJF',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 1.5 Project Finance',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CAC15PJF_LOAD01',
        python_callable=CAC15PJF_LOAD01
    )
    
    task2 = PythonOperator(
        task_id='CAC15PJF_XFRM01',
        python_callable=CAC15PJF_XFRM01
    )

    task3 = PythonOperator(
        task_id='CAC15PJF_AP01_MPI_DER_PJF',
        python_callable=CAC15PJF_AP01_MPI_DER_PJF    
    )

    task4 = PythonOperator(
        task_id='CAC15PJF_VALD01',
        python_callable=CAC15PJF_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='CAC15PJF_GEN01',
        python_callable=CAC15PJF_GEN01      
    ) 
