...
def CAC17SHM_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_SHM')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT010701'])
...
def CAC17SHM_AP01_MPI_DER_SHM():
    ...
    function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status)
def CAC17SHM_VALD01():
    # query data with condition RDT vilidation and return msr_prd_id where not True 
    try:
        #read config files
        config = function.read_config_psql('DER_SHM')
    except Exception as e:
    ...
    try:
        #uery data with condition RDT vilidation and return msr_prd_id where not True 
        #validation rule get report column msr_prd_id

        #If credit account is not an account of lead manager of syndicated loan or club loan, then share lending member is not required to report.
        result_CNSHM001 = pd.read_sql(config['RDT010703'] ,conn_psql) 

        #If credit account is an account of lead manager of syndicated loan or club loan and the credit account has been effective since ‘2023-01-01’, then share lending member must be reported.
        result_CMSHM001 = pd.read_sql(config['RDT010704'] ,conn_psql) 

        #If a share lending member record exists, then a credit account must exist.
        result_RISHM001 = pd.read_sql(config['RDT010705'] ,conn_psql) 

        #If a share lending member record exists, then a counterparty record of the share lending member must exist.
        result_RISHM002 = pd.read_sql(config['RDT010706'] ,conn_psql) 
    ...
    #validation success
    if (result_CNSHM001['msr_prd_id'].count() == 0 and
        result_CMSHM001['msr_prd_id'].count() == 0 and
        result_RISHM001['msr_prd_id'].count() == 0 and
        result_RISHM002['msr_prd_id'].count() == 0 ):
        ...
        try:
            if result_CNSHM001['msr_prd_id'].count() != 0:
                function.upload_csv_to_minio(result_CNSHM001,'{0}{1}/CAC17SHM_CNSHM001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC17SHM_CNSHM001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMSHM001['msr_prd_id'].count() != 0:
                function.upload_csv_to_minio(result_CMSHM001,'{0}{1}/CAC17SHM_CMSHM001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC17SHM_CMSHM001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RISHM001['msr_prd_id'].count() != 0:
                function.upload_csv_to_minio(result_RISHM001,'{0}{1}/CAC17SHM_RISHM001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC17SHM_RISHM001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RISHM002['msr_prd_id'].count() != 0:
                function.upload_csv_to_minio(result_RISHM002,'{0}{1}/CAC17SHM_RISHM002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC17SHM_RISHM002.csv fail Line", exc_tb.tb_lineno)
...
def CAC17SHM_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT010707'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_CAC_17_DER_SHM',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 1.7 Share Lending Member',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CAC17SHM_LOAD01',
        python_callable=CAC17SHM_LOAD01
    )
    
    task2 = PythonOperator(
        task_id='CAC17SHM_XFRM01',
        python_callable=CAC17SHM_XFRM01
    )

    task3 = PythonOperator(
        task_id='CAC17SHM_AP01_MPI_DER_SHM',
        python_callable=CAC17SHM_AP01_MPI_DER_SHM    
    )

    task4 = PythonOperator(
        task_id='CAC17SHM_VALD01',
        python_callable=CAC17SHM_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='CAC17SHM_GEN01',
        python_callable=CAC17SHM_GEN01      
    ) 
