...
def CAC18ACXAC_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_ACXAC')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT010801'])
...
def CAC18ACXAC_AP01_MPI_DER_ACXAC():
    try:
        #read config files
        config = function.read_config_psql('DER_ACXAC')
    except Exception as e:
...
def CAC18ACXAC_VALD01():
    ...
    try:
        ...
        #[Previous Account Id] must not be the same as [Account Id].
        result_CNACXAC001 = pd.read_sql(config['RDT010802'] ,conn_psql) 

        #If an account x account record exists, then a credit account record of the related account must exist.
        result_RIACXAC001 = pd.read_sql(config['RDT010803'] ,conn_psql)

        #If an account x account record exists, then a credit account record of the previous account must exist.
        result_RIACXAC002 = pd.read_sql(config['RDT010804'] ,conn_psql) 
    ...
    #validation success
    if (result_CNACXAC001['Account Id'].count() == 0 and
        result_RIACXAC001['Account Id'].count() == 0 and
        result_RIACXAC002['Account Id'].count() == 0):
        ...
        try:
            if result_CNACXAC001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_CNACXAC001,'{0}{1}/CAC18ACXAC_CNACXAC001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC18ACXAC_CNACXAC001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RIACXAC001['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_RIACXAC001,'{0}{1}/CAC18ACXAC_RIACXAC001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC18ACXAC_RIACXAC001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RIACXAC002['Account Id'].count() != 0:
                function.upload_csv_to_minio(result_RIACXAC002,'{0}{1}/CAC18ACXAC_RIACXAC002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CAC18ACXAC_RIACXAC002.csv fail Line", exc_tb.tb_lineno)

    #update batch   
    status = config['STATUS_SUCCESS']
    function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status)      

def CAC18ACXAC_GEN01(): 
    try:
        #read config files
        config = function.read_config_psql('DER_ACXAC')
    except Exception as e:
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT010805'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_CAC_18_DER_ACXAC',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 1.8 Account x Account',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CAC18ACXAC_LOAD01',
        python_callable=CAC18ACXAC_LOAD01
    )
    
    task2 = PythonOperator(
        task_id='CAC18ACXAC_XFRM01',
        python_callable=CAC18ACXAC_XFRM01
    )

    task3 = PythonOperator(
        task_id='CAC18ACXAC_AP01_MPI_DER_ACXAC',
        python_callable=CAC18ACXAC_AP01_MPI_DER_ACXAC    
    )

    task4 = PythonOperator(
        task_id='CAC18ACXAC_VALD01',
        python_callable=CAC18ACXAC_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='CAC18ACXAC_GEN01',
        python_callable=CAC18ACXAC_GEN01      
    ) 
