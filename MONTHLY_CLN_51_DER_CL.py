...
def CLN51CL_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_CL')
...
def CLN51CL_AP01_MPI_DER_CL():
    ...
    #mapping Entity ID from table 4.7
    set_a = ['ENT_ID']
    set_b = ['ENT_ID']
    where_a = ['ENT_ID']
    where_b = ['AC_ID']
    function.update_table_using_another_table(config['TABLE'], config['TABLE_MAPPING'], 1, 1, set_a, set_b, where_a, where_b, config['RDT600008'], config['MSR_PRD_ID'])
...
def CLN51CL_VALD01(): 
    # query data with condition RDT vilidation and return Credit Line Id where not True 
    try:
        #read config files
        config = function.read_config_psql('DER_CL')
    except Exception as e:
    ...
    try:
        #uery data with condition RDT vilidation and return Credit Line Id where not True 
        #validation rule get report column Credit Line Id

        #[Credit Line Effective Date] must be after or the same as [Credit Line Approve Date].
        result_CNCL001 = pd.read_sql(config['RDT050103'] ,conn_psql) 

        #[Credit Line Amount in Original Currency] must be greater than or equal to 0.
        result_CNCL002 = pd.read_sql(config['RDT050104'] ,conn_psql) 

        #If [Root Credit Line Id] is the same as [Credit Line Id], it means that this is a top level of the credit line, then [Parent Credit Line Id] must be blank and [Level Number] must be equal to 0. Otherwise, [Parent Credit Line Id] cannot be blank and [Level Number] must be greater than 0.
        result_CMCL001 = pd.read_sql(config['RDT050105'] ,conn_psql) 

        #If a credit line record exists, then a counterparty entity record for the credit line must exist.
        result_RICL001 = pd.read_sql(config['RDT050106'] ,conn_psql) 

        #If a credit line record exists, then a credit line record for the parent credit line must exist.
        result_RICL002 = pd.read_sql(config['RDT050107'] ,conn_psql)  

        #If a credit line record exists, then a credit line record for the root credit line must exist.
        result_RICL003 = pd.read_sql(config['RDT050108'] ,conn_psql) 

        #If a credit line record exists and application id is not blank, then an application record for the credit line must exist.
        result_RICL004 = pd.read_sql(config['RDT050109'] ,conn_psql) 
    ...
    #validation success
    if (result_CNCL001['Credit Line Id'].count() == 0 and
        result_CNCL002['Credit Line Id'].count() == 0 and
        result_CMCL001['Credit Line Id'].count() == 0 and
        result_RICL001['Credit Line Id'].count() == 0 and
        result_RICL002['Credit Line Id'].count() == 0 and
        result_RICL003['Credit Line Id'].count() == 0 and
        result_RICL004['Credit Line Id'].count() == 0 ):
        ...
        try:
            if result_CNCL001['Credit Line Id'].count() != 0:
                function.upload_csv_to_minio(result_CNCL001,'{0}{1}/CLN51CL_CNCL001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CLN51CL_CNCL001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CNCL002['Credit Line Id'].count() != 0:
                function.upload_csv_to_minio(result_CNCL002,'{0}{1}/CLN51CL_CNCL002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CLN51CL_CNCL002.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMCL001['Credit Line Id'].count() != 0:
                function.upload_csv_to_minio(result_CMCL001,'{0}{1}/CLN51CL_CMCL001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CLN51CL_CMCL001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RICL001['Credit Line Id'].count() != 0:
                function.upload_csv_to_minio(result_RICL001,'{0}{1}/CLN51CL_RICL001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CLN51CL_RICL001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RICL002['Credit Line Id'].count() != 0:
                function.upload_csv_to_minio(result_RICL002,'{0}{1}/CLN51CL_RICL002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CLN51CL_RICL002.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RICL003['Credit Line Id'].count() != 0:
                function.upload_csv_to_minio(result_RICL003,'{0}{1}/CLN51CL_RICL003.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CLN51CL_RICL003.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RICL004['Credit Line Id'].count() != 0:
                function.upload_csv_to_minio(result_RICL004,'{0}{1}/CLN51CL_RICL004.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CLN51CL_RICL004.csv fail Line", exc_tb.tb_lineno)
...
def CLN51CL_GEN01():
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT050110'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_CLN_51_DER_CL',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 5.1 Credit Line',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CLN51CL_LOAD01',
        python_callable=CLN51CL_LOAD01
    )

    task2 = PythonOperator(
        task_id='CLN51CL_XFRM01',
        python_callable=CLN51CL_XFRM01    
    )

    task3 = PythonOperator(
        task_id='CLN51CL_AP01_MPI_DER_CL',
        python_callable=CLN51CL_AP01_MPI_DER_CL      
    ) 

    task4 = PythonOperator(
        task_id='CLN51CL_VALD01',
        python_callable=CLN51CL_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='CLN51CL_GEN01',
        python_callable=CLN51CL_GEN01
    )
