...
def CLN52CLLT_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_CLLT')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT050201'])
...
def CLN52CLLT_AP01_MPI_DER_CLLT ():
    try:
        #read config files
        config = function.read_config_psql('DER_CLLT')
    except Exception as e:
...
def CLN52CLLT_VALD01():
    ...
    try:
	    ...
	    #If a credit line loan type record exists, then a credit line record must exist, as the two data entities describe the element of credit line.
        result_RICLLT001  = pd.read_sql(config['RDT050202'] ,conn_psql) 
    ...
    #validation success
    if ( result_RICLLT001 ['Credit Line Id'].count() == 0 ):
        ...
        try:
            if result_RICLLT001 ['Credit Line Id'].count() != 0:
                function.upload_csv_to_minio(result_RICLLT001 ,'{0}{1}/CLN52CLLT_RICLLT001 .csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CLN52CLLT_RICLLT001 .csv fail Line", exc_tb.tb_lineno)
...
def CLN52CLLT_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT050203'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_CLN_52_DER_CLLT',
         schedule_interval='@monthly',
         default_args=default_args,
         description='Monthly : 5.2 Credit Line Loan Type',
         catchup=False) as dag:
 
    task_1 = PythonOperator(
        task_id='CLN52CLLT_LOAD01',
        python_callable=CLN52CLLT_LOAD01
    )

    task_2 = PythonOperator(
        task_id='CLN52CLLT_XFRM01',
        python_callable=CLN52CLLT_XFRM01
        
    )

    task_3 = PythonOperator(
        task_id='CLN52CLLT_AP01_MPI_DER_CLLT',
        python_callable=CLN52CLLT_AP01_MPI_DER_CLLT 
        
    )

    task_4 = PythonOperator(
        task_id='CLN52CLLT_VALD01',
        python_callable=CLN52CLLT_VALD01
        
    )

    task_5 = PythonOperator(
        task_id='CLN52CLLT_GEN01',
        python_callable=CLN52CLLT_GEN01
