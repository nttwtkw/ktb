...
def CLN56OVD_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_OVD')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT050601'])
...
def CLN56OVD_VALD01():
    ...
    try:
        ...
        #If an override or deviation record exists, then a credit line record for must exist.
        result_RIOVD001 = pd.read_sql(config['RDT050602'] ,conn_psql) 
    ...
    #validation success
    if (result_RIOVD001['Credit Line Id'].count() == 0):
        ...
        try:
            if result_RIOVD001['Credit Line Id'].count() != 0:
                function.upload_csv_to_minio(result_RIOVD001,'{0}{1}/CLN56OVD_RIOVD001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CLN56OVD_RIOVD001.csv fail Line", exc_tb.tb_lineno)
...
def CLN56OVD_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT050603'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_CLN_56_DER_OVD',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 5.6 Override or Deviation',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CLN56OVD_LOAD01',
        python_callable=CLN56OVD_LOAD01
    )
    
    task2 = PythonOperator(
        task_id='CLN56OVD_XFRM01',
        python_callable=CLN56OVD_XFRM01
    )

    task3 = PythonOperator(
        task_id='CLN56OVD_AP01_MPI_DER_OVD',
        python_callable=CLN56OVD_AP01_MPI_DER_OVD    
    )

    task4 = PythonOperator(
        task_id='CLN56OVD_VALD01',
        python_callable=CLN56OVD_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='CLN56OVD_GEN01',
        python_callable=CLN56OVD_GEN01      
    ) 
