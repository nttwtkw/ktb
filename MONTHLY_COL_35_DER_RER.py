...
def COL35RER_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_RER')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT030501'])
...
def COL35RER_VALD01():
    ...
    try:
        #query data with condition RDT vilidation and return Land Collateral Id where not True 

        #If a real estate relationship record exists, then a land record must exist.
        result_RIRER001 = pd.read_sql(config['RDT030502'] ,conn_psql) 

        #If a real estate relationship record exists, then a building record must exist.
        result_RIRER002 = pd.read_sql(config['RDT030503'] ,conn_psql) 
    ...
    #validation success
    if (result_RIRER001['Land Collateral Id'].count() == 0 and
        result_RIRER002['Land Collateral Id'].count() == 0):
        ...
        try:
            if result_RIRER001['Land Collateral Id'].count() != 0:
                function.upload_csv_to_minio(result_RIRER001,'{0}{1}/COL35RER_RIRER001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio COL35RER_RIRER001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RIRER002['Land Collateral Id'].count() != 0:
                function.upload_csv_to_minio(result_RIRER002,'{0}{1}/COL35RER_RIRER002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio COL35RER_RIRER002.csv fail Line", exc_tb.tb_lineno)
...
def COL35RER_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT030504'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_COL_35_DER_RER',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 3.5 Real Estate Relationship',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='COL35RER_LOAD01',
        python_callable=COL35RER_LOAD01
    )
    
    task2 = PythonOperator(
        task_id='COL35RER_XFRM01',
        python_callable=COL35RER_XFRM01
    )

    task3 = PythonOperator(
        task_id='COL35RER_AP01_MPI_DER_RER',
        python_callable=COL35RER_AP01_MPI_DER_RER    
    )

    task4 = PythonOperator(
        task_id='COL35RER_VALD01',
        python_callable=COL35RER_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='COL35RER_GEN01',
        python_callable=COL35RER_GEN01      
    ) 
