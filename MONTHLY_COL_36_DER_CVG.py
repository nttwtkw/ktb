...
def COL36CVG_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_CVG')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT030601'])
...
def COL36CVG_VALD01():
    ...
    try:
        #query data with condition RDT vilidation and return Valuation Group Id and Collateral Id where not True 

        #If a collateral valuation group record exists, then a collateral record must exist.
        result_RICVG001 = pd.read_sql(config['RDT030602'], conn_psql)
    ...
    #validation success
    if (result_RICVG001['Valuation Group Id'].count() == 0 ):
        ...
        try:
            if result_RICVG001['Valuation Group Id'].count() != 0:
                function.upload_csv_to_minio(result_RICVG001,'{0}{1}/COL36CVG_RICVG001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio COL36CVG_RICVG001.csv fail Line", exc_tb.tb_lineno)
...
def COL36CVG_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT030603'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_COL_36_DER_CVG',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 3.6 Collateral Valuation Group',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='COL36CVG_LOAD01',
        python_callable=COL36CVG_LOAD01
    )
    
    task2 = PythonOperator(
        task_id='COL36CVG_XFRM01',
        python_callable=COL36CVG_XFRM01
    )

    task3 = PythonOperator(
        task_id='COL36CVG_AP01_MPI_DER_CVG',
        python_callable=COL36CVG_AP01_MPI_DER_CVG   
    )

    task4 = PythonOperator(
        task_id='COL36CVG_VALD01',
        python_callable=COL36CVG_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='COL36CVG_GEN01',
        python_callable=COL36CVG_GEN01     
    ) 
