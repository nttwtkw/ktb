...
def COL38DG_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_GE')
...
def COL38DG_VALD01():
    ...
    try:
        #query data with condition RDT vilidation and return Guarantor or Endorser Group Id, Guarantor or Endorser Name and Counterparty Id where not True 

        #Either [Guarantor or Endorser Name] or [Counterparty Id] cannot be blank.
        result_CMGE001 = pd.read_sql(config['RDT030803'], conn_psql)

        #If a guarantor or endorser record exists, then a counterparty record of the guarantor or endorser must exist.
        result_RIGE001 = pd.read_sql(config['RDT030804'], conn_psql) 
    ...
    #validation success
    if (result_CMGE001['Counterparty Id'].count() == 0 and
        result_RIGE001['Counterparty Id'].count() == 0 ):
        ...
        try:
            if result_CMGE001['Credit Line Id'].count() != 0:
                function.upload_csv_to_minio(result_CMGE001,'{0}{1}/COL38DG_CMGE001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio COL38DG_CMGE001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RIGE001['Credit Line Id'].count() != 0:
                function.upload_csv_to_minio(result_RIGE001,'{0}{1}/COL38DG_RIGE001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name,table)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio COL38DG_RIGE001.csv fail Line", exc_tb.tb_lineno)
...
def COL38DG_GEN01():
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT030805'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_COL_38_DER_GE',
         schedule_interval='@monthly',
         default_args=default_args,
         description='Monthly: 3.8 Guarantor or Endorser',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='COL38DG_LOAD01',
        python_callable=COL38DG_LOAD01
    )

    task2 = PythonOperator(
        task_id='COL38DG_XFRM01',
        python_callable=COL38DG_XFRM01
    )

    task3 = PythonOperator(
        task_id='COL38DG_AP01_MPI_DER_DG',
        python_callable=COL38DG_AP01_MPI_DER_DG
    )

    task4 = PythonOperator(
        task_id='COL38DG_VALD01',
        python_callable=COL38DG_VALD01
    ) 

    task5 = PythonOperator(
        task_id='COL38DG_GEN01',
        python_callable=COL38DG_GEN01
    )
