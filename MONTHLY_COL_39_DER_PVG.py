...
def COL39PVG_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_PVG')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT030901'])
...
def COL39PVG_VALD01():
    ...
    try:
        #uery data with condition RDT vilidation and return Valuation Group Id where not True 

        #If a pledge valuation group record exists, then a valuation group record must exist.
        result_RIPVG001 = pd.read_sql(config['RDT030903'] ,conn_psql) 
    ...
    #validation success
    if (result_RIPVG001['Valuation Group Id'].count() == 0):
        ...
        try:
            if result_RIPVG001['Valuation Group Id'].count() != 0:
                function.upload_csv_to_minio(result_RIPVG001,'{0}{1}/COL39PVG_RIPVG001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio COL39PVG_RIPVG001.csv fail Line", exc_tb.tb_lineno)
...
def COL39PVG_GEN01():
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT030903'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_COL_39_DER_PVG',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 3.9 Pledge Valuation Group',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='COL39PVG_LOAD01',
        python_callable=COL39PVG_LOAD01
    )
    
    task2 = PythonOperator(
        task_id='COL39PVG_XFRM01',
        python_callable=COL39PVG_XFRM01
    )

    task3 = PythonOperator(
        task_id='COL39PVG_AP01_MPI_DER_PVG',
        python_callable=COL39PVG_AP01_MPI_DER_PVG    
    )

    task4 = PythonOperator(
        task_id='COL39PVG_VALD01',
        python_callable=COL39PVG_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='COL39PVG_GEN01',
        python_callable=COL39PVG_GEN01      
    ) 
