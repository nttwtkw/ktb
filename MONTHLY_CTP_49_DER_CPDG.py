...
def CTP49CPDG_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_CPDG')
...
def CTP49CPDG_AP01_MPI_DER_CPDG():
    ...
    try:
        function.insert_table_into_postgresql(table,query,int(config['CHUNKSIZE']),config['TABLE'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
def CTP49CPDG_VALD01():
    ...
    try:
        ...
        result_RICPDG001 = pd.read_sql(config['RDT040902'] ,conn_psql) 

        #If a counterparty debtor group record exists, then a counterparty x id record must exist.
        result_RICPDG002 = pd.read_sql(config['RDT040903'] ,conn_psql) 
...
def CTP49CPDG_GEN01():
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT040904'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
    ...
    try:
        ...
        function.upload_csv_to_minio(df,path, int(config['CHUNKSIZE']),s3,bucket_name,config['TABLE'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_CTP_49_DER_CPDG',
