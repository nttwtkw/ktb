...
def INT62INTR_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('DER_INTR')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT060201'])
...
def INT62INTR_XFRM01():
    ...
    status = config['STATUS_SUCCESS']
    function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status)
def INT62INTR_AP01_MPI_DER_INTR():
    try:
        #read config files
        config = function.read_config_psql('DER_INTR')
    except Exception as e:
    ...
    status = config['STATUS_SUCCESS']
    function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status)
def INT62INTR_VALD01():
    # query data with condition RDT vilidation and return Reference Rate Id where not True 
    try:
        #read config files
        config = function.read_config_psql('DER_INTR')
    except Exception as e:
    ...
    try:
        #uery data with condition RDT vilidation and return Reference Rate Id where not True 
        #validation rule get report column Reference Rate Id

        #If [Interest Rate Type] is “Other Interest Rate Type”, then [Interest Rate Type Description] cannot be blank.
        result_CMINTR001 = pd.read_sql(config['RDT060203'] ,conn_psql) 

        #If [Interest Rate Type] is “Term Deposit”, “BIBOR”, “SIBOR”, “USD LIBOR / Fallback SOFR”, “GBP LIBOR / Fallback SONIA”, “CHF LIBOR / Fallback SARON”, “EUR LIBOR / Fallback ESTER”, “JPY LIBOR / Fallback TONA”, “TIBOR”, “HIBOR”, “EURIBOR”, “THBFIX / Fallback THBFIX” or “SOR / Fallback SOR”, then [Tenor of Reference Rate] cannot be blank and must be greater than 0 and [Tenor of Reference Rate Term Unit] cannot be blank.
        result_CMINTR002 = pd.read_sql(config['RDT060204'] ,conn_psql) 
    ...
    #validation success
    if (result_CMINTR001['Reference Rate Id'].count() == 0 and
        result_CMINTR002['Reference Rate Id'].count() == 0 ):
        ...
        try:
            if result_CMINTR001['Reference Rate Id'].count() != 0:
                function.upload_csv_to_minio(result_CMRTR001,'{0}{1}/INT62INTR_CMRTR001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio INT62INTR_CMRTR001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMINTR002['Reference Rate Id'].count() != 0:
                function.upload_csv_to_minio(result_RIRTR001,'{0}{1}/INT62INTR_RIRTR001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio INT62INTR_RIRTR001.csv fail Line", exc_tb.tb_lineno)
...
def INT62INTR_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT060205'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('MONTHLY_INT_62_DER_INTR',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 6.2 Interest Reference',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='INT62INTR_LOAD01',
        python_callable=INT62INTR_LOAD01
    )
    
    task2 = PythonOperator(
        task_id='INT62INTR_XFRM01',
        python_callable=INT62INTR_XFRM01
    )

    task3 = PythonOperator(
        task_id='INT62INTR_AP01_MPI_DER_INTR',
        python_callable=INT62INTR_AP01_MPI_DER_INTR    
    )

    task4 = PythonOperator(
        task_id='INT62INTR_VALD01',
        python_callable=INT62INTR_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='INT62INTR_GEN01',
        python_callable=INT62INTR_GEN01      
    ) 
