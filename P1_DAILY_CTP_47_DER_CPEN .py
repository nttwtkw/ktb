...
def CTP47CPEN_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('P1_DER_CPEN_D')
...
def CTP47CPEN_AP01_DPI_DER_CPEN():
    try:
        #read config files
        config = function.read_config_psql('P1_DER_CPEN_D')
    except Exception as e:
...
def CTP47CPEN_VALD01():
    ...
    try:
        ...
        result_RICPEN001 = pd.read_sql(config['RDT040702'] ,conn_psql) 
...
def CTP47CPEN_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT040703'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
    ...
    try:
        ...
        function.upload_csv_to_minio_2(df,path, int(config['CHUNKSIZE']),s3,bucket_name,'RDTDBA.DER_CPEN_D')
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('P1_DAILY_CTP_47_DER_CPEN',
         schedule_interval='@daily', 
         default_args=default_args,
         description='Daily: 4.7 Counterparty Entity Daily',
    ...
    task3 = PythonOperator(
        task_id='CTP47CPEN_AP01_DPI_DER_CPEN',
        python_callable=CTP47CPEN_AP01_DPI_DER_CPEN    
    )
