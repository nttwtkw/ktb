...
def CAC111PRT_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('P1_DER_PRT')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT011101'])
...
def CAC111PRT_AP01_MPI_DER_PRT():
    try:
        #read config files
        config = function.read_config_psql('P1_DER_PRT')
    except Exception as e:
...
def CAC111PRT_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT011103'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('P1_MONTHLY_CAC_111_DER_PRT',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 1.11 Portfolio',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CAC111PRT_LOAD01',
        python_callable=CAC111PRT_LOAD01
    )
    
    task2 = PythonOperator(
        task_id='CAC111PRT_XFRM01',
        python_callable=CAC111PRT_XFRM01
    )

    task3 = PythonOperator(
        task_id='CAC111PRT_AP01_MPI_DER_PRT',
        python_callable=CAC111PRT_AP01_MPI_DER_PRT 
    )

    task4 = PythonOperator(
        task_id='CAC111PRT_GEN01',
        python_callable=CAC111PRT_GEN01  
    ) 
