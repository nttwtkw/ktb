...
def COL33LND_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('P1_DER_LND')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT030301'])
...
def COL33LND_VALD01():
    ...
    try:
        ...
        result_CNLND001 = pd.read_sql(config['RDT030303'] ,conn_psql) 

        #If [Country] is ‘TH’, then [Land Ownership Flag] cannot be blank.
        result_CMLND001 = pd.read_sql(config['RDT030304'] ,conn_psql) 

        #If [Country] is ‘TH’, then either [DOPA Location] or [DOL Location] cannot be blank.
        result_CMLND002 = pd.read_sql(config['RDT030305'] ,conn_psql) 

        #If a land record exists, then a collateral record must exist.
        result_RILND001 = pd.read_sql(config['RDT030306'] ,conn_psql) 
    ...
    #validation success
    if (result_CNLND001['Collateral Id'].count() == 0 and
        result_CMLND001['Collateral Id'].count() == 0 and
        result_CMLND002['Collateral Id'].count() == 0 and
        result_RILND001['Collateral Id'].count() == 0 ):
        ...
        try:
            if result_CNLND001['Collateral Id'].count() != 0:
                function.upload_csv_to_minio(result_CNLND001,'{0}{1}/COL33LND_CNLND001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio COL33LND_CNLND001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMLND001['Collateral Id'].count() != 0:
                function.upload_csv_to_minio(result_CMLND001,'{0}{1}/COL33LND_CMLND001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio COL33LND_CMLND001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMLND002['Collateral Id'].count() != 0:
                function.upload_csv_to_minio(result_CMLND002,'{0}{1}/COL33LND_CMLND002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio COL33LND_CMLND002.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RILND001['Collateral Id'].count() != 0:
                function.upload_csv_to_minio(result_RILND001,'{0}{1}/COL33LND_RILND001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio COL33LND_RILND001.csv fail Line", exc_tb.tb_lineno)
...
def COL33LND_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT030307'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('P1_MONTHLY_COL_33_DER_LND',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 3.3 Land',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='COL33LND_LOAD01',
        python_callable=COL33LND_LOAD01
    )
    
    task2 = PythonOperator(
        task_id='COL33LND_XFRM01',
        python_callable=COL33LND_XFRM01
    )

    task3 = PythonOperator(
        task_id='COL33LND_AP01_MPI_DER_LND',
        python_callable=COL33LND_AP01_MPI_DER_LND    
    )

    task4 = PythonOperator(
        task_id='COL33LND_VALD01',
        python_callable=COL33LND_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='COL33LND_GEN01',
        python_callable=COL33LND_GEN01      
    ) 
