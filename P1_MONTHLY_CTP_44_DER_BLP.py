...
def CTP44BLP_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('P1_DER_BLP')
...
def CTP44BLP_VALD01():
    ...
    try:
        ...
        result_RIBLP001 = pd.read_sql(config['RDT040402'] ,conn_psql) 

        #If [Main Factory Country] is ‘TH’, then [Main Factory Location] cannot be blank
        result_CMBLP002 = pd.read_sql(config['RDT040403'] ,conn_psql) 

        #[Labor] must be greater than or equal to 0
        result_CNBLP001 = pd.read_sql(config['RDT040404'] ,conn_psql) 

        #[Domestic Income in Baht] must be greater than or equal to 0
        result_CNBLP002 = pd.read_sql(config['RDT040405'] ,conn_psql) 
        ...
        result_CMBLP003 = pd.read_sql(config['RDT040406'] ,conn_psql) 
    ...
    #validation success
    if (result_RIBLP001['Counterparty Id'].count() == 0 and
        result_CMBLP002['Counterparty Id'].count() == 0 and
        result_CNBLP001['Counterparty Id'].count() == 0 and
        result_CNBLP002['Counterparty Id'].count() == 0 and
        result_CMBLP003['Counterparty Id'].count() == 0 ):
        ...
        try:
            if result_RIBLP001['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_RIBLP001,'{0}{1}/CTP44BLP_RIBLP001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP44BLP_RIBLP001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMBLP002['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CMBLP002,'{0}{1}/CTP44BLP_CMBLP002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP44BLP_CMBLP002.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CNBLP001['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CNBLP001,'{0}{1}/CTP44BLP_CNBLP001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP44BLP_CNBLP001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CNBLP002['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CNBLP002,'{0}{1}/CTP44BLP_CNBLP002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP44BLP_CNBLP002.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMBLP003['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CMBLP003,'{0}{1}/CTP44BLP_CMBLP003.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP44BLP_CMBLP003.csv fail Line", exc_tb.tb_lineno)
...
def CTP44BLP_GEN01():
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT040407'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('P1_MONTHLY_CTP_44_DER_BLP',
