...
def CTP45PLP_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('P1_DER_PLP')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT040501'])
...
def CTP45PLP_VALD01():
    ...
    try:
        ...
        #[Other Debt Burden in Baht] must be greater than or equal to 0.
        result_CNPLP001 = pd.read_sql(config['RDT040502'] ,conn_psql) 

        #If account purpose of the credit account is for individual consumption,
        #then a personal loan profile is required to report.
        result_CMPLP001 = pd.read_sql(config['RDT040503'] ,conn_psql) 

        #If [Employment Status] is not “Unemployed”, then [Employment Characteristic] cannot be blank.
        #Otherwise, [Occupation] must be “Housewife / Homemaker”, “Student”, “Retiree” and “Others”.
        result_CMPLP002 = pd.read_sql(config['RDT040504'] ,conn_psql) 

        #If [Employee Loan Flag] in Credit Account Detail (DER_CACD) is equal to 0,
        #then [Income in Baht must be greater than or equal to 0
        result_CMPLP004 = pd.read_sql(config['RDT040505'] ,conn_psql)

        #If Either [Identification Type] in Counterparty x Id (DER_CPID) is ‘Personal Id’ 
        #or [Country of Residence] in Ordinary Counterparty (DER_OCP) is ‘TH’,
        #then [Contact Location] cannot be blank.
        result_CMPLP005 = pd.read_sql(config['RDT040506'] ,conn_psql)

        #If a personal loan profile record exists,
        #then a counterparty x id record for the personal loan profile must exist.
        result_RIPLP001 = pd.read_sql(config['RDT040507'] ,conn_psql)
    ...
    #validation success
    if (result_CNPLP001['Counterparty Id'].count() == 0 and
        result_CMPLP001['Counterparty Id'].count() == 0 and
        result_CMPLP002['Counterparty Id'].count() == 0 and
        result_CMPLP004['Counterparty Id'].count() == 0 and
        result_CMPLP005['Counterparty Id'].count() == 0 and
        result_RIPLP001['Counterparty Id'].count() == 0):
        ...
        try:
            if result_CNPLP001['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CNPLP001,'{0}{1}/CTP45PLP_CNPLP001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP45PLP_CNPLP001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMPLP001['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CMPLP001,'{0}{1}/CTP45PLP_CMPLP001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP45PLP_CMPLP001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMPLP002['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CMPLP002,'{0}{1}/CTP45PLP_CMPLP002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP45PLP_CMPLP002.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMPLP004['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CMPLP004,'{0}{1}/CTP45PLP_CMPLP004.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP45PLP_CMPLP004.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_CMPLP005['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_CMPLP005,'{0}{1}/CTP45PLP_CMPLP005.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP45PLP_CMPLP005.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RIPLP001['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_RIPLP001,'{0}{1}/CTP45PLP_RIPLP001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP45PLP_RIPLP001.csv fail Line", exc_tb.tb_lineno)
...
def CTP45PLP_GEN01():
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT040508'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('P1_MONTHLY_CTP_45_DER_PLP',
         schedule_interval='@monthly',
         default_args=default_args,
         description='Monthly: 4.5 Personal Loan Profile',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CTP45PLP_LOAD01',
        python_callable=CTP45PLP_LOAD01
    )

    task2 = PythonOperator(
        task_id='CTP45PLP_XFRM01',
        python_callable=CTP45PLP_XFRM01
    )

    task3 = PythonOperator(
        task_id='CTP45PLP_AP01_MPI_DER_PLP',
        python_callable=CTP45PLP_AP01_MPI_DER_PLP
    )

    task4 = PythonOperator(
        task_id='CTP45PLP_VALD01',
        python_callable=CTP45PLP_VALD01
    ) 

    task5 = PythonOperator(
        task_id='CTP45PLP_GEN01',
        python_callable=CTP45PLP_GEN01
    )
