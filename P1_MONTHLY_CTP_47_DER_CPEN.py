...
def CTP47CPEN_LOADDB201():
    try:
        #read config files
        config = function.read_config_psql('P1_DER_CPEN')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT040701'])
...
def CTP47CPEN_AP01_MTI_STG_ENT():
    #insert data from .parquet to staging on psql
    try:
        #read config files
        config = function.read_config_psql('P1_DER_CPEN')
    except Exception as e:
...
def CTP47CPEN_AP01_MTI_STG_MAS_ENT_DTL():
...
def CTP47CPEN_LOADCPEN01():
    ...
    try:
        #read table,convert dataframe to .parquet and upload to minio
        query = function.replace_value(config['RDT040703'], ['{MSR_PRD_ID}'], [config['MSR_PRD_ID']])
        function.psql_to_minio(bucket, query, file_name, path)
        #con_db2.close()
    except Exception as e:
        print(e)
        print('Connection psql fail Line 39')
...
def CTP47CPEN_AP01_MPI_DER_CPEN():
    ...
    try:
        ...
        table = function.read_table_psql_modify(config['RDT040704'])
    except  Exception as e:
        print(e)
        print('Insert data to table on PSQL fail Line 144')
        
        function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        raise AirflowFailException("Cannot inset data to mas_ent table on PSQL fail Line 144", exc_tb.tb_lineno)
        
    try:
        #insert data to mas_ent_dtl table on PostgreSQL 
        table = function.read_table_psql_modify(config['RDT040705'])
    except  Exception as e:
        print(e)
        print('Insert data to table on PSQL fail Line 144')
        
        function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
        raise AirflowFailException("Inset data to mas_ent_dtl table on PSQL fail Line 144", exc_tb.tb_lineno)
...
def CTP47CPEN_VALD01():
    ...
    try:
        ...
        #If a counterparty entity record exists, then a counterparty x id record must exist.
        result_RICPEN001 = pd.read_sql(config['RDT040706'] ,conn_psql)
    ...
    if (    result_RICPEN001['Counterparty Id'].count() == 0):
        ...
        try:
            if result_RICPEN001['Counterparty Id'].count() != 0:
                function.upload_csv_to_minio(result_RICPEN001,'{0}{1}/CTP47CPEN_RICPEN001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
...
def CTP47CPEN_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT040707'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
        ...
        for object in bucket.objects.filter(Prefix = file_success):
            ...
            if check_validation_fail == file_success:
                ...
                try:
                    ...
                    function.update_config_psql(list_parm_values, list_parm_nm, config['RDT600003'], "P1_"+config['TABLE03'].split(".")[1])
                except Exception as e:
                    exc_type, exc_obj, exc_tb = sys.exc_info()
                    print(e)
...
with DAG('P1_MONTHLY_CTP_47_DER_CPEN',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 4.7 Counterparty Entity',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CTP47CPEN_LOADDB201',
        python_callable=CTP47CPEN_LOADDB201
    )
    
    task2 = PythonOperator(
        task_id='CTP47CPEN_AP01_MTI_STG_ENT',
        python_callable=CTP47CPEN_AP01_MTI_STG_ENT
    )
    
    task3 = PythonOperator(
        task_id='CTP47CPEN_AP01_MTI_STG_MAS_ENT_DTL',
        python_callable=CTP47CPEN_AP01_MTI_STG_MAS_ENT_DTL
    )
    
    task4 = PythonOperator(
        task_id='CTP47CPEN_LOADCPEN01',
        python_callable=CTP47CPEN_LOADCPEN01
    )
