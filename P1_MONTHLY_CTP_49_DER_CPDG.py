...
def CTP49CPDG_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('P1_DER_CPDG')
    ...
    try:
        ...
        df,con_db2 = function.read_db2(config['RDT040901'])
...
def CTP49CPDG_AP01_MPI_DER_CPDG():
    try:
        #read config files
        config = function.read_config_psql('P1_DER_CPDG')
    except Exception as e:
    ...
    status = config['STATUS_SUCCESS']
    function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status)
def CTP49CPDG_VALD01():
    # query data with condition RDT vilidation and return Debtor Group Id where not True 
    try:
        #read config files
        config = function.read_config_psql('P1_DER_CPDG')
    except Exception as e:
    ...
    try:
        #uery data with condition RDT vilidation and return Debtor Group Id where not True 
        #validation rule get report column Debtor Group Id

        #If a counterparty debtor group record exists, then a debtor group record must exist.
        result_RICPDG001 = pd.read_sql(config['RDT040903'] ,conn_psql) 

        #If a counterparty debtor group record exists, then a counterparty x id record must exist.
        result_RICPDG002 = pd.read_sql(config['RDT040904'] ,conn_psql) 
    ...
    #validation success
    if (result_RICPDG001['Debtor Group Id'].count() == 0 and
        result_RICPDG002['Debtor Group Id'].count() == 0 ):
        ...
        try:
            if result_RICPDG001['Debtor Group Id'].count() != 0:
                function.upload_csv_to_minio(result_RICPDG001,'{0}{1}/CTP49CPDG_RICPDG001.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP49CPDG_RICPDG001.csv fail Line", exc_tb.tb_lineno)

        try:
            if result_RICPDG002['Debtor Group Id'].count() != 0:
                function.upload_csv_to_minio(result_RICPDG002,'{0}{1}/CTP49CPDG_RICPDG002.csv'.format(config['VALD_PATH'],unixtime),chunksize,s3,bucket_name)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print(e)
            function.update_btch(config['RDT600001'],config['RDT600002'],task_name,config['BTCH_ID'],number_id_job,date_process,status,True)
            raise AirflowFailException("upload_csv_to_minio CTP49CPDG_RICPDG002.csv fail Line", exc_tb.tb_lineno)
...
def CTP49CPDG_GEN01(): 
    ...
    try:
        #gen files
        df = function.read_table_psql(config['RDT040905'])
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        print(e)
...
with DAG('P1_MONTHLY_CTP_49_DER_CPDG',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Monthly: 4.9 Counterparty Debtor Group',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='CTP49CPDG_LOAD01',
        python_callable=CTP49CPDG_LOAD01
    )
    
    task2 = PythonOperator(
        task_id='CTP49CPDG_XFRM01',
        python_callable=CTP49CPDG_XFRM01
    )

    task3 = PythonOperator(
        task_id='CTP49CPDG_AP01_MPI_DER_CPDG',
        python_callable=CTP49CPDG_AP01_MPI_DER_CPDG    
    )

    task4 = PythonOperator(
        task_id='CTP49CPDG_VALD01',
        python_callable=CTP49CPDG_VALD01      
    ) 

    task5 = PythonOperator(
        task_id='CTP49CPDG_GEN01',
        python_callable=CTP49CPDG_GEN01      
    ) 
