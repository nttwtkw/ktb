...
def STG_LOAN_ACCM_LOAD01():
    try:
        #read config files
        config = function.read_config_psql('STG_LOAN_ACCM')
...
with DAG('STAGING_CREDIT_ACCOUNT',
         schedule_interval='@monthly', 
         default_args=default_args,
         description='Staging Monthly: Credit Account Staging ',
         catchup=False) as dag:
 
    task1 = PythonOperator(
        task_id='STG_LOAN_ACCM_LOAD01',
        python_callable=STG_LOAN_ACCM_LOAD01
    )
